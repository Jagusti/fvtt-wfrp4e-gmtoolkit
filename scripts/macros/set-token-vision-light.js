...
async function setTokenVisionLight () {
  ...
  const dialogContent = `
        <form> 
          <div class="form-group">
            ...
            <select id="visiontype" name="visiontype"> 
          ...
          <div class="form-group">
            ...
            <select id="lightsource" name="lightsource">
              <option value="none"> 
  ...
  await foundry.applications.api.DialogV2.wait({
    window: { title: game.i18n.localize("GMTOOLKIT.Dialog.SetVisionLight.Title") },
    rejectClose: false,
    ...
    close: async () => {
      if (result) applyTokenChanges(result)
    }
  })
}


async function applyTokenChanges (updates) {
  for ( const token of canvas.tokens.controlled ) {
    ...
    let lightSource = updates.lightsource
    let visionType = updates.visiontype  // TODO: default vision option based on condition -> trait -> talent. Issue Log: https://github.com/Jagusti/fvtt-wfrp4e-gmtoolkit/issues/42
    ...
    switch (visionType) {
      ...
      case "darkVision":
        ...
        break
      case "nightVision":
        const actorNightVision
          = token.actor.has(game.i18n.localize("NAME.NightVision", "trait"))
        ...
        if (
          ...
          if (actorNightVision === undefined) {
            ...
            switch (actorNightVision.type) {
              case "trait":
                advNightVision = 1
                break
              case "talent":
                advNightVision += actorNightVision.system.Advances
                break
            }
          }
